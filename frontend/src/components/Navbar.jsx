...
const Navbar = () => {
  ...
  return (
    <div className='flex justify-between py-3 px-5 sm:px-20 border-b border-gray-300 '>
      ...
      <div>
        <button onClick={() => navigate("/admin")} className='px-5 py-2 bg-green-500 rounded-full cursor-pointer hover:bg-green-400 hover:scale-103 transition-all duration-300 flex items-center justify-center gap-1'>{token ? "Dashboard" : "Login"} <FaArrowRightLong className='hidden sm:inline-block ' /></button>
      </div>
    </div>
  )
}
